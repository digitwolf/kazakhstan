...
**Where to eat (day.html):** `day.html` renders a per-day "Where to Eat" section from a `window.DAYS[].eats` array, grouped by meal slot (breakfast/coffee/lunch/snack/dinner…) with the `area`; each pick shows name, cuisine, ⭐ rating, the short `why`, the verified Wikimedia dish `photo` (lazy-loaded, hidden on error) and a "View on Google Maps ↗" link (`map`). The kid pick (`kid:true`) is flagged with a "👦 Kid pick — simple food" badge for the picky 6-year-old. The data is sourced from the local-guide files `tour/daily-guides/day-NN.md` — each carries a fenced ```json block (`{d, title, overnight, schedule, todo[], meals[]}`); `gen_data.py` loops the files generically, extracts the json, and attaches the `meals` as `eats` (+ the `todo` as `localTodo`) onto the matching `window.DAYS` entry by `d`. Days with no guide file get no `eats` and render nothing. Keep `data.js` key-free; keep it byte-reproducible.

**Themed "trail/thread" highlights (e.g. the Ramen Trail):** a curated cross-route thread (a set of stops that share a theme) is surfaced as a structured data object parsed from a single `tour/` overview section — not hardcoded in HTML. Pattern: `gen_data.py` parses the section table (e.g. "The Ramen Trail" in `tour/00-overview.md`) into `window.RAMEN_TRAIL` (`{title, subtitle, intro, note, bookend, stops[]}`, each stop carrying `day`, `slot`, `city`, `pref`, `style`, `styleDesc`, marquee `shop`/`shopUrl` + `alts[]` key-free Google-Maps links, and a verified Wikimedia dish `photo` — reuse the day-guide thumbs); `index.html` renders it as a themed section whose cards **deep-link each stop to `day.html?d=N`**. Flag the marquee days on the day pages from data, never hardcoded: attach a compact `ramenTrail[]` to the matching `window.DAYS` entries (a multi-entry day, e.g. Day 9, is the "two-ramen day"), render it as a 🍜 callout in `day.html`, and additionally set `trail:true` on any `eats` pick whose Google-Maps `cid` matches a trail shop so it gets a per-pick 🍜 badge. Invent nothing — every field comes from the overview section; verify all dish photos are HTTP 200; keep `data.js` key-free and byte-reproducible.
