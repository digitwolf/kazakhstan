...
8. **Mileage + routes.** Per-day riding miles (≈1,546 total) shown on day cards and overlaid on each destination hero; each day links to a **Google Maps start→end route** (waypoints for multi-stop days); each day card deep-links to its destination page.
